...
import { syncTrackedMediaObjectsFromGhl } from "@/lib/media/store";
import { listMediaObjects, parseMediaFilter, parseMediaScope, parseMediaSort } from "@/lib/media/store";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const items = await listMediaObjects(listInput);
    ...
    const refreshed = await listMediaObjects(listInput);
